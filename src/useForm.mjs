...
const useForm = () => {
  ...
  const readElements = useCallback(() => {
    ...
    for (let idx = 0; idx < formElements.length; idx++) {
      ...
      const msg= el.getAttribute('data-formiga-validity') || ''
      const val= el.getAttribute('data-formiga-value') || ''
      const valid = msg==''
  ...
  useEffect(() => {
    ...
    formRef.current.addEventListener('formiga-form-change', formValidityListener)
